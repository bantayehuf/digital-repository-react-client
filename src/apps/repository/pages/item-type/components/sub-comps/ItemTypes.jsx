...
let timer;
export default class ItemTypes extends React.Component {
  constructor(props) {
    super(props);
  ...
  fetchData = async (options) => {
    ...
    if (!urlInitiated && options.searching) {
      ...
      url = `${url}/items_type/search?q=${options.q}&page_size=${this.state.numberOfRows}`;
      urlInitiated = true;
    } else if (
    ...
    if (!urlInitiated && options.pageNation) {
      if (this.state.search.searching && this.state.search.searchQuery !== "") {
        url = `${url}/items_type/search?q=${this.state.search.searchQuery}&page_size=${this.state.numberOfRows}&page=${options.selectedPage}`;
      } else {
        url = `${url}/items_type/list?page_size=${this.state.numberOfRows}&page=${options.selectedPage}`;
      }
      urlInitiated = true;
    } else if (!urlInitiated && options.numberOfRows) {
      if (this.state.search.searching && this.state.search.searchQuery !== "") {
        url = `${url}/items_type/search?q=${this.state.search.searchQuery}&page_size=${options.numberOfRowsValue}`;
      } else {
        url = `${url}/items_type/list?page_size=${options.numberOfRowsValue}`;
      }
      urlInitiated = true;
    } else if (!urlInitiated) {
      url = `${url}/items_type/list?page_size=${this.state.numberOfRows}`;
      urlInitiated = true;
    }
    ...
    try {
      ...
      if (response.status === 200) {
        const data = response.data.results.map((itemType) => {
          const desc = itemType.description ? (
            itemType.description
          ) : (
            <span key={itemType.id} className="text-sr">
          ...
          return {
            ...
            description: desc,
            created_by: (
              <ItemCreatedByViewModal createdBy={itemType.created_by_info} />
            ),
            created_at: itemType.date_of_created,
          };
        });
    ...
    return (
      <>
        <ActionHeader
          ...
          urlForDeleteItems="/repository/items_type/delete"
        />
        {
              ...
              {this.state.resultFound ? (
                <>
                  ...
                  <CRow className="overflow-auto">
                    <DataTable
                      columns={columns}
                      data={this.state.itemTypes.results}
                    />
                  </CRow>
